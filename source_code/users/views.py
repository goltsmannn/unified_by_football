...
from map.models import Activity, Favorites
...
class RegisterUserAPIView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer: UserRegisterSerializer) -> Response:
        ...
        try:
            send_mail(
                'Confirm your email',
                f'Click this link to confirm your email: http://unifiedbyfootball.ru/{activation_url}',
...
def subscribed_users(request, placemark_id: int) -> Response:
    ...
    """
    id_list = Favorites.objects.filter(placemark__id=placemark_id).values_list('user', flat=True)
